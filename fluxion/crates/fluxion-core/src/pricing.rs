...
use crate::components::{FixedPriceData, SpotPriceData, TimeBlockPrice};
...
) -> crate::components::PriceAnalysis {
    use crate::components::{PriceAnalysis, PriceRange};
...
mod tests {
    ...
    fn test_parse_spot_price_response() {
        ...
        let data = parse_spot_price_response(&response).unwrap();

        // Should expand 3 hourly blocks to 12 15-min blocks
        assert_eq!(data.time_block_prices.len(), 12);
        assert_eq!(data.block_duration_minutes, 15);

        // Check first hour's blocks
        assert_eq!(data.time_block_prices[0].price_czk_per_kwh, 0.40);
        assert_eq!(data.time_block_prices[1].price_czk_per_kwh, 0.40);
        assert_eq!(data.time_block_prices[2].price_czk_per_kwh, 0.40);
        assert_eq!(data.time_block_prices[3].price_czk_per_kwh, 0.40);

        // Check second hour's blocks
        assert_eq!(data.time_block_prices[4].price_czk_per_kwh, 0.45);
    }

    #[test]
    fn test_parse_spot_price_response_with_15min_blocks() {
        let response = json!({
            "entity_id": "sensor.test_price",
            "state": "0.45",
            "last_updated": "2025-10-02T10:00:00Z",
            "attributes": {
                "forecast": [
                    {
                        "start": "2025-10-02T10:00:00Z",
                        "end": "2025-10-02T10:15:00Z",
                        "price": 0.40
                    },
                    {
                        "start": "2025-10-02T10:15:00Z",
                        "end": "2025-10-02T10:30:00Z",
                        "price": 0.45
                    }
                ]
            }
        ...
        // Should keep 15-min blocks as-is
        assert_eq!(data.time_block_prices.len(), 2);
        assert_eq!(data.block_duration_minutes, 15);
        assert_eq!(data.time_block_prices[0].price_czk_per_kwh, 0.40);
        assert_eq!(data.time_block_prices[1].price_czk_per_kwh, 0.45);
    }

    #[test]
    fn test_has_price_data_changed() {
        let now = Utc::now();
        ...
        assert!(!has_price_data_changed(&data, now));
        assert!(has_price_data_changed(&data, later));
    }

    #[test]
    fn test_create_fixed_price_data_hourly() {
        let buy = vec![0.05; 24];
        let sell = vec![0.08; 24];

        let data = create_fixed_price_data(buy, sell).unwrap();

        // Should expand to 96 blocks
        assert_eq!(data.buy_prices.len(), 96);
        assert_eq!(data.sell_prices.len(), 96);
        assert_eq!(data.block_duration_minutes, 15);
    }

    #[test]
    fn test_create_fixed_price_data_15min() {
        let buy = vec![0.05; 96];
        let sell = vec![0.08; 96];

        let data = create_fixed_price_data(buy, sell).unwrap();

        // Should keep 96 blocks
        assert_eq!(data.buy_prices.len(), 96);
        assert_eq!(data.sell_prices.len(), 96);
        assert_eq!(data.block_duration_minutes, 15);
    }
    ...
    #[test]
    fn test_create_fixed_price_data_invalid_length() {
        let buy = vec![0.05; 12]; // Invalid
        let sell = vec![0.08; 24];

        let result = create_fixed_price_data(buy, sell);
        assert!(result.is_err());
    }

    #[test]
    fn test_convert_to_15min_blocks() {
        let now = Utc::now();
        let blocks = vec![
            TimeBlockPrice {
                block_start: now,
        ...
        let result = convert_to_15min_blocks(blocks);

        // 2 hours = 8 15-min blocks
        assert_eq!(result.len(), 8);
        assert_eq!(result[0].price_czk_per_kwh, 0.40);
        assert_eq!(result[0].duration_minutes, 15);
        assert_eq!(result[4].price_czk_per_kwh, 0.50);
    }

    #[test]
    fn test_analyze_prices_basic() {
        let now = Utc::now();
        let blocks = vec![
            TimeBlockPrice {
                block_start: now,
            ...
            TimeBlockPrice {
                ...
                duration_minutes: 15,
                price_czk_per_kwh: 0.40, // Cheapest
            },
            TimeBlockPrice {
                block_start: now + chrono::Duration::minutes(30),
                duration_minutes: 15,
                price_czk_per_kwh: 0.60, // Most expensive
            },
            TimeBlockPrice {
                block_start: now + chrono::Duration::minutes(45),
                duration_minutes: 15,
                price_czk_per_kwh: 0.45,
            },
        ];

        let analysis = analyze_prices(&blocks, 1, 1, true, true);

        // 1 hour = 4 blocks for charge, 4 blocks for discharge
        assert_eq!(analysis.charge_blocks.len(), 4);
        assert_eq!(analysis.discharge_blocks.len(), 4);

        // Check that cheapest block is identified
        assert!(analysis.charge_blocks.contains(&1)); // Index 1 has cheapest price

        // Check that most expensive block is identified
        assert!(analysis.discharge_blocks.contains(&2)); // Index 2 has highest price

        // Check price range
        assert_eq!(analysis.price_range.min_czk_per_kwh, 0.40);
        assert_eq!(analysis.price_range.max_czk_per_kwh, 0.60);
        assert!((analysis.price_range.avg_czk_per_kwh - 0.4875).abs() < 0.001);
    }

    #[test]
    fn test_analyze_prices_spot_disabled() {
        let now = Utc::now();
        let blocks = vec![
            TimeBlockPrice {
                block_start: now,
        ...
        // Disable both buy and sell spot pricing
        let analysis = analyze_prices(&blocks, 1, 1, false, false);

        // Should not identify any blocks
        assert_eq!(analysis.charge_blocks.len(), 0);
        assert_eq!(analysis.discharge_blocks.len(), 0);

        // But price range should still be calculated
        assert_eq!(analysis.price_range.min_czk_per_kwh, 0.40);
        assert_eq!(analysis.price_range.max_czk_per_kwh, 0.60);
    }

    #[test]
    fn test_analyze_prices_empty_data() {
        let analysis = analyze_prices(&[], 1, 1, true, true);
    ...
    #[test]
    fn test_analyze_prices_partial_data() {
        let now = Utc::now();
        // Only 2 blocks but requesting 2 hours worth (8 blocks)
        let blocks = vec![
            TimeBlockPrice {
                block_start: now,
                duration_minutes: 15,
        ...
        let analysis = analyze_prices(&blocks, 2, 2, true, true);

        // Should only identify available blocks (2), not 8
        assert_eq!(analysis.charge_blocks.len(), 2);
        assert_eq!(analysis.discharge_blocks.len(), 2);
    }

    #[test]
    fn test_analyze_prices_24h_pattern() {
        let now = Utc::now();
        // Simulate 24 hours (96 blocks) with varying prices
        let mut blocks = Vec::new();

        for hour in 0..24 {
            let price = match hour {
                0..=5 => 0.30,   // Night - cheap
                6..=8 => 0.45,   // Morning
                9..=16 => 0.50,  // Day
                17..=20 => 0.60, // Peak - expensive
                _ => 0.40,       // Evening
            };
            ...
            for block in 0..4 {
                blocks.push(TimeBlockPrice {
                    block_start: now
                        ...
                        + chrono::Duration::minutes(block * 15),
                    duration_minutes: 15,
                    price_czk_per_kwh: price,
                });
            }
        ...
        // Request 4 hours of charging (16 blocks) and 2 hours of discharging (8 blocks)
        let analysis = analyze_prices(&blocks, 4, 2, true, true);

        assert_eq!(analysis.charge_blocks.len(), 16); // 4 hours
        assert_eq!(analysis.discharge_blocks.len(), 8); // 2 hours

        // All charge blocks should be in night hours (0-5, blocks 0-23)
        for &block_idx in &analysis.charge_blocks {
            assert!(
                block_idx < 24,
                "Charge block {} not in night hours",
                block_idx
            );
        }

        // All discharge blocks should be in peak hours (17-20, blocks 68-83)
        for &block_idx in &analysis.discharge_blocks {
            assert!(
                (68..84).contains(&block_idx),
                "Discharge block {} not in peak hours",
                block_idx
            );
        }
    }
