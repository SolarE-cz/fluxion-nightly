...
pub fn validate_config(config: &SystemConfig) -> (Vec<ValidationIssue>, Vec<ValidationIssue>) {
    ...
    if config.system_config.update_interval_secs < 5 {
        errors.push(ValidationIssue {
            field: "system.update_interval_secs".to_string(),
            message: "Update interval must be at least 5 seconds".to_string(),
            severity: "error".to_string(),
        });
    } else if config.system_config.update_interval_secs < 30 {
        warnings.push(ValidationIssue {
            field: "system.update_interval_secs".to_string(),
            message: "Short update interval may cause high CPU usage".to_string(),
    ...
    if control.min_battery_soc < 0.0 || control.min_battery_soc > 100.0 {
        errors.push(ValidationIssue {
            field: "control.min_battery_soc".to_string(),
            message: "Minimum SOC must be between 0% and 100%".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.max_battery_soc < 0.0 || control.max_battery_soc > 100.0 {
        errors.push(ValidationIssue {
            field: "control.max_battery_soc".to_string(),
            message: "Maximum SOC must be between 0% and 100%".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.min_battery_soc > control.max_battery_soc {
        errors.push(ValidationIssue {
            field: "control.min_battery_soc".to_string(),
            message: "Minimum SOC cannot be higher than maximum SOC".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.hardware_min_battery_soc < 0.0 || control.hardware_min_battery_soc > 20.0 {
         warnings.push(ValidationIssue {
            field: "control.hardware_min_battery_soc".to_string(),
            message: "Hardware minimum SOC is unusually high (>20%) or invalid".to_string(),
            severity: "warning".to_string(),
        });
    }
    ...
    if control.min_battery_soc < control.hardware_min_battery_soc {
         errors.push(ValidationIssue {
            field: "control.min_battery_soc".to_string(),
            message: format!("Minimum SOC cannot be lower than hardware limit ({}%)", control.hardware_min_battery_soc),
            severity: "error".to_string(),
        });
    }
    ...
    if control.force_charge_hours > 24 {
        errors.push(ValidationIssue {
            field: "control.force_charge_hours".to_string(),
            message: "Force charge hours cannot exceed 24".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.force_discharge_hours > 24 {
        errors.push(ValidationIssue {
            field: "control.force_discharge_hours".to_string(),
            message: "Force discharge hours cannot exceed 24".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.force_charge_hours + control.force_discharge_hours > 24 {
        errors.push(ValidationIssue {
            field: "control.force_charge_hours".to_string(),
            message: "Total force hours cannot exceed 24".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.evening_peak_start_hour > 23 {
        errors.push(ValidationIssue {
            field: "control.evening_peak_start_hour".to_string(),
            message: "Evening peak hour must be between 0 and 23".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if control.battery_capacity_kwh <= 0.0 {
         errors.push(ValidationIssue {
            field: "control.battery_capacity_kwh".to_string(),
    ...
    if control.battery_efficiency <= 0.0 || control.battery_efficiency > 1.0 {
         errors.push(ValidationIssue {
            field: "control.battery_efficiency".to_string(),
            message: "Battery efficiency must be between 0 and 1.0".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if pricing.spot_buy_fee_czk < 0.0 {
        errors.push(ValidationIssue {
            field: "pricing.spot_buy_fee_czk".to_string(),
            message: "Buy fee cannot be negative".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if pricing.spot_sell_fee_czk < 0.0 {
        errors.push(ValidationIssue {
            field: "pricing.spot_sell_fee_czk".to_string(),
            message: "Sell fee cannot be negative".to_string(),
            severity: "error".to_string(),
        });
    }
    ...
    if strategies.winter_peak_discharge_enabled {
        if strategies.winter_peak_min_spread_czk < 0.0 {
            errors.push(ValidationIssue {
                field: "strategies.winter_peak_min_spread_czk".to_string(),
                message: "Minimum spread cannot be negative".to_string(),
                severity: "error".to_string(),
            });
        }
        ...
        if strategies.winter_peak_solar_window_start >= strategies.winter_peak_solar_window_end {
             errors.push(ValidationIssue {
                field: "strategies.winter_peak_solar_window_start".to_string(),
                message: "Solar window start must be before end".to_string(),
                severity: "error".to_string(),
            });
        }
    }
    ...
    // Winter Adaptive
    if strategies.winter_adaptive_enabled {
        if strategies.winter_adaptive_ema_period_days < 1 {
             errors.push(ValidationIssue {
                field: "strategies.winter_adaptive_ema_period_days".to_string(),
                message: "EMA period must be at least 1 day".to_string(),
                severity: "error".to_string(),
            });
        }
    }
...
mod tests {
    ...
    fn test_force_hours_limit() {
        ...
        let (errors, _) = validate_config(&config);
        assert!(errors.iter().any(|e| e.field == "control.force_charge_hours"));
    ...
    fn test_update_interval_warning() {
        ...
        let (errors, warnings) = validate_config(&config);
        assert!(errors.is_empty());
        assert!(warnings.iter().any(|w| w.field == "system.update_interval_secs"));
    }
    ...
    fn test_strategy_validation() {
        ...
        let (errors, _) = validate_config(&config);
        assert!(errors.iter().any(|e| e.field == "strategies.winter_peak_min_spread_czk"));
    }
}
